//! `marksman-provider-swift` — the Swift language provider as a standalone sidecar process
//! (the `CI_PROVIDER=sidecar` wire; `lang-rust`'s sidecar is the pattern).
//! Usage: `marksman-provider-swift --root /path/to/repo` (then it serves stdin/stdout).
...
fn main() {
    ...
    if let Some(missing) = lang_swift::gate_missing() {
        eprintln!("[marksman-provider-swift] {missing}");
        std::process::exit(1);
    }
    ...
    if let Err(e) = ci_proto::serve_stdio(provider, outline) {
        eprintln!("[marksman-provider-swift] serve error: {e}");
        std::process::exit(1);
    }
}
