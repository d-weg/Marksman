//! `marksman-provider-php` — the PHP language provider as a standalone sidecar process
//! (the `CI_PROVIDER=sidecar` wire; `lang-rust`/`lang-java`'s sidecar is the pattern).
//! Usage: `marksman-provider-php --root /path/to/repo` (then it serves stdin/stdout).
...
fn main() {
    ...
    if let Some(missing) = lang_php::gate_missing(&root) {
        eprintln!("[marksman-provider-php] {missing}");
        std::process::exit(1);
    }
    ...
    if let Err(e) = ci_proto::serve_stdio(provider, outline) {
        eprintln!("[marksman-provider-php] serve error: {e}");
        std::process::exit(1);
    }
}
