...
Marksman's gate (the type-check verdict) and rename engine need each language's real
...
Without a runtime, Marksman runs the toolchain on the host (`HostSandbox`) exactly as before —
container mode is opt-in and additive.

## The helper

Everything below is driven by one script — [`scripts/marksman-images.sh`](../scripts/marksman-images.sh):

```
scripts/marksman-images.sh check          # runtime present? which images exist? are the pins in sync?
scripts/marksman-images.sh build          # build every image
scripts/marksman-images.sh build ts rust  # build only what your repos need
scripts/marksman-images.sh list           # what each image holds, and its size
```

## Step 1 — a container runtime

Install one of `container` (Apple's), `docker`, `podman`, or `nerdctl`. Marksman picks the
...
```
scripts/marksman-images.sh check
# runtime:   docker
```

If it says `runtime: NONE`, install a runtime — with `CI_SANDBOX=oci` and no runtime,
Marksman warns at startup and stays on the host (loud, never silent).
...
```
scripts/marksman-images.sh build ts rust
```
...
```json
{ "command": "marksman-mcp", "args": ["--root", "."], "env": { "CI_SANDBOX": "oci" } }
```

That's it. Marksman starts one warm container per language on demand, reuses it across
operations, and removes it on exit. The repo and the system temp dir are bind-mounted at
their **identical host paths**, so every path in requests, replies, and `.marksman/`
artifacts is valid on both sides.

## Verifying / troubleshooting

`scripts/marksman-images.sh check` is the one-stop status:

- **`missing marksman-<lang>`** — build it; until then, an operation on that language errors
...
|---|---|---|
| `marksman-rust` | cargo + rust-analyzer | gate + rename |
| `marksman-ts` | scip-typescript + tsgo + typescript | gate + rename **+ indexer** |
| `marksman-java` | JDK 21 + jdtls | gate + rename |
| `marksman-php` | php + phpstan + phpactor | gate + rename |
| `marksman-swift` | the Swift toolchain | gate + rename |

TypeScript is the one language whose **read** path (the SCIP indexer) also needs the
toolchain, so `marksman-ts` carries `scip-typescript` in addition to the gate/rename tier —
