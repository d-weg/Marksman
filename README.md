...
Peashooter is a local-first [Model Context Protocol](https://modelcontextprotocol.io) server, written in Rust, that gives an AI coding agent a reliable way to work with a codebase:
...
Coding agents spend a large share of their tokens searching for context, and blind string edits routinely break builds. Peashooter addresses both: retrieval hands the agent the right line ranges directly, and every edit is verified by the language's own compiler — across the changed files *and* the files that depend on them — before it is committed. When a wide change is rejected, the rejection enumerates every affected site with a ready-to-apply fix, so the type checker does the site-finding.

On a 10-task end-to-end agent benchmark, an agent using Peashooter completed every task correctly at **~45% lower cost and ~42% less wall-clock time** than the same agent without it (a repo-wide type rename: 3 turns instead of 22). Methodology, ablations, and caveats: [docs/benchmarks.md](docs/benchmarks.md).
...
Every response is explicit about which guarantee it carries (`gated: true/false`) — Peashooter never silently degrades a gate. Per-language details, toolchains, and status: [docs/languages/](docs/languages/).
...
- **Rust** (stable), to build Peashooter — <https://rustup.rs>
...
```bash
target/release/peashooter index /path/to/your/repo   # writes .peashooter/
# optional sanity check:
target/release/peashooter retrieve /path/to/your/repo "where is the rate limiter"
```
...
The server operates on the repository it is launched in (its working directory); pass `--root /path/to/repo` or set `PEASHOOTER_ROOT` to point it elsewhere.
...
its own devices a coding agent falls back on grep + manual edits, and you never see Peashooter's
...
```text
You have peashooter MCP tools. They are DEFERRED — load them FIRST, in ONE call, with their FULL names:
...
To place the model by hand, drop it at the default path and Peashooter finds it with no configuration:

```bash
mkdir -p ~/.peashooter/models/potion-code-16M
curl -fL --output-dir ~/.peashooter/models/potion-code-16M \
...
```bash
scripts/peashooter-images.sh check          # runtime? images? pins in sync?
scripts/peashooter-images.sh build ts rust  # build just what your repos need
...
```
peashooter index    <repo>                              # build / refresh the index (.peashooter/)
peashooter retrieve <repo> "<task>" [--top N] [--json]  # query the index
peashooter doctor   [<repo>]                            # per-language dependency report;
                                                      # exit 1 if unhealthy
```

## Configuration

Peashooter works with no configuration. For non-default setups:

| Variable | Meaning | Default |
|---|---|---|
| `PEASHOOTER_ROOT` | Repository root for the MCP server | current directory |
| `CI_MODEL_DIR` | Embedding model directory | `~/.peashooter/models/potion-code-16M` |
...
An optional `peashooter.config.json` in the repository root overrides retrieval and index settings (top-N, fusion weights, per-language SCIP, …).

## Project status

Peashooter is under active development. TypeScript and Rust are the reference implementations — fully gated and benchmark-validated. Java, PHP, and Swift have gated edits landed with benchmark validation in progress. The remaining languages ride the generic tree-sitter provider with syntax-level gating until their compiler gate lands.
